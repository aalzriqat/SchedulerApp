...
import { BackendShift } from '../../api/apiService'; // AdminScheduleView no longer needed here
import {
  fetchUserSchedule,
  clearEmployeeScheduleErrors,
  fetchFilteredAvailableShifts, // Import the new thunk
} from '../../store/slices/employeeScheduleSlice';
import { createNewSwapRequest, CreateSwapRequestPayload } from '../../store/slices/swapSlice';
...
const AvailableSwapsScreen = () => {
  ...
  const {
    ...
    // Use new state fields from Redux for filtered shifts
  ...
  const themedSelectedShiftBg = useThemeColor({}, 'tint'); // For selected item in list

  useEffect(() => {
    console.log("[AvailSwaps] Current User:", currentUser);
    if (currentUser?.id) {
  ...
  const handleFindShifts = () => { 
    ...
    console.log(`[AvailSwaps] Dispatching fetchFilteredAvailableShifts for week ${week}, excluding user ${currentUser.id}.`);
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
      <ThemedView style={[styles.container, {backgroundColor: themedBackgroundColor}]}>
        ...
        {/* Keyboard dismiss wrapper starts here */}
        <View style={{flex: 1}}>
      ...
      {selectedWeekFilter === '' && !isLoadingFilteredSwappable && !errorFilteredSwappable && (!filteredSwappableShifts || filteredSwappableShifts.length === 0) && (
        ...
        {/* Keyboard dismiss wrapper ends here */}

        {/* Modal remains outside the dismiss wrapper */}
...
const styles = StyleSheet.create({
  ...
  requestButton: { // This style is no longer used on individual items, but kept for modal button
    paddingVertical: 10, 
  ...
  requestButtonText: { // Used in modal
    fontWeight: 'bold',
    fontSize: 16, 
  },
  errorText: {
  ...
  ownShiftItemSelected: {
    // backgroundColor: '#e0e0e0', // Apply themed selection color inline
    // Example: borderColor: themedPrimaryButtonBg, borderWidth: 2
  },
  selectedShiftItem: { // Style for the selected shift in the FlatList
    // Example: borderColor: 'green', // Use themed color
    // borderWidth: 2,
  },
  modalList: {
    maxHeight: 150, 
